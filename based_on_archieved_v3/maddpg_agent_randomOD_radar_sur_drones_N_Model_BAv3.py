...
class MADDPG:
    ...
    def update_myown(self, i_episode, total_step_count, UPDATE_EVERY, single_eps_critic_cal_record, transfer_learning, use_allNeigh_wRadar, use_selfATT_with_radar, wandb=None, full_observable_critic_flag=False, use_GRU_flag=False):
        ...
        for agent in range(var_iteration):
            if full_observable_critic_flag and isinstance(self.actors, list):
                ...
                stacked_elem_3 = padding_surrounding_nei(batch.states_surrounding_nei, self.n_agents, self.device)
                # stacked_elem_3 = []
                # for one_in_batch in batch.states_surrounding_nei:
                #     cur_states_surround_nei = np.zeros((self.n_agents-1, one_in_batch.shape[1]))
                #     cur_states_surround_nei[-len(one_in_batch):, :] = one_in_batch
                #     stacked_elem_3.append(cur_states_surround_nei)
                # stacked_elem_3 = torch.Tensor(np.array(stacked_elem_3)).to(self.device)
                ...
                next_stacked_elem_3 = padding_surrounding_nei(batch.next_states_surrounding_nei, self.n_agents, self.device)

                dones_stacked = torch.stack(batch.dones)
                reward_batch = torch.stack(batch.rewards)
                action_batch = torch.stack(batch.actions)
                non_final_next_states_actorin = [next_stacked_elem_0, next_stacked_elem_3, next_stacked_elem_2]
            ...
            if full_observable_critic_flag:
                ...
                if use_GRU_flag:
                    ...
                    action_i = self.actors([stacked_elem_0, stacked_elem_3, stacked_elem_2])
                else:
    ...
    def choose_action(self, state, cur_total_step, cur_episode, step, mini_noise_eps, noise_start_level, actor_hiddens, use_allNeigh_wRadar, use_selfATT_with_radar, own_obs_only, noisy=True, use_GRU_flag=False):
        ...
        all_obs_surAgent = [torch.from_numpy(np.vstack(single_agent_nei)).to(self.device)
                            for single_agent_nei in state[3]]
        ...
        for i in range(self.n_agents):
            ...
            if use_GRU_flag:
                ...
                act = self.actors([sb.unsqueeze(0), sb_surAgent.unsqueeze(0), sb_grid.unsqueeze(0)])  # include batch size in action, due to the use of unsqueeze(0)
            elif use_allNeigh_wRadar:
                if own_obs_only:
