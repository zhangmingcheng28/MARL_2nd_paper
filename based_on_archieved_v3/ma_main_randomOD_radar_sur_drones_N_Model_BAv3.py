...
import pickle
import wandb
from parameters_randomOD_radar_sur_drones_N_Model_BAv3 import initialize_parameters
from maddpg_agent_randomOD_radar_sur_drones_N_Model_BAv3 import MADDPG
from iddpg_attention import Iddpg_att
from utils_randomOD_radar_sur_drones_N_Model_BAv3 import *
...
def main(args):
    ...
    #                    use_single_portion_selfATT, use_selfATT_with_radar, use_allNeigh_wRadar, own_obs_only,
    #                    env.normalizer, device)
    elif args.algo == "iddpg_att":
        model = Iddpg_att(actor_dim, critic_dim, n_actions, actor_hidden_state, gru_history_length, n_agents, args,
                       criticNet_lr, actorNet_lr, GAMMA, TAU, full_observable_critic_flag, use_GRU_flag,
                       use_single_portion_selfATT, use_selfATT_with_radar, use_allNeigh_wRadar, own_obs_only,
                       env.normalizer, device)
    ...
    episode = 0
    current_row = 0
    excel_file_path = '../archieved_model_80%_sortie_reach/experience_replay_data.xlsx'
                ...
                if len(gru_history) >= gru_history_length:
                    ...
                    if full_observable_critic_flag:
                        ...
                        for i in range(len(one_agent_next_obs)):
                            ...
                            model.memory.push(one_agent_obs[i][0], one_agent_obs[i][1], one_agent_obs[i][2], one_agent_obs[i][3], ac_tensor[i, :], one_agent_next_obs[i][0], one_agent_next_obs[i][1], one_agent_next_obs[i][2], one_agent_next_obs[i][3], rw_tensor[i], done_tensor[i], history_tensor[:,i,:],
                                              cur_actor_hiddens[i, :], next_actor_hiddens[i,:])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--algo', default="maddpg", type=str, help ="commnet/bicnet/maddpg/maddpg_combine,iddpg_att")
