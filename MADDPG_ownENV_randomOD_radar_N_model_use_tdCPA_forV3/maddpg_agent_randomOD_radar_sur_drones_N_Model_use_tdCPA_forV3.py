...
from Nnetworks_randomOD_radar_sur_drones_N_Model_use_tdCPA_forV3 import *
from Utilities_own_randomOD_radar_sur_drones_N_Model_use_tdCPA_forV3 import *
import torch
from copy import deepcopy
from torch.optim import Adam
from memory_randomOD_radar_sur_drones_N_Model_use_tdCPA_forV3 import ReplayMemory, Experience
...
class MADDPG:
        ...
        if full_observable_critic_flag:
            ...
            if use_GRU_flag:
                ...
                self.pretrained_mask_all_mask_head.load_state_dict(
                    torch.load('F:\githubClone\LLM_MARL\multi_agent_AAC_v2\mask_model_fully_masked_wTanh_v2.pth'))
                ...
                self.pretrained_mask_inside_mask.load_state_dict(
                    torch.load('F:\githubClone\LLM_MARL\multi_agent_AAC_v2\mask_model_eps20000_in_mask_wTanh_v2.pth'))
        ...
        for agent in range(var_iteration):
            ...
            if full_observable_critic_flag:
                ...
                if use_GRU_flag:
                    ...
                    non_final_next_actions = self.actors_target([non_final_next_states_actorin[0],
                                                                 non_final_next_states_actorin[2], next_mask_batch])
    ...
    def choose_action(self, state, cur_total_step, cur_episode, step, mini_noise_eps, noise_start_level, actor_hiddens, use_allNeigh_wRadar, use_selfATT_with_radar, own_obs_only, use_nearestN_neigh_wRadar, all_agent_dict, shared_one_actor_one_critic, noisy=True, use_GRU_flag=False):
        ...
        if isinstance(actor_hiddens, list):
            gru_history_input = torch.FloatTensor(np.stack(actor_hiddens)).unsqueeze(0).to(self.device)  # batch x no_agent x feature_length
        else:
        ...
        for i in range(self.n_agents):
            ...
            sb_grid = obs_grid[i]
            agent_masks_record.append(all_agent_dict[i].action_mask)
            ...
            if use_GRU_flag:
                ...
                act = self.actors([sb.unsqueeze(0), sb_grid.unsqueeze(0), mask_encoding])
        ...
        return actions.data.cpu().numpy(), noise_value, gru_history_input.squeeze(0).data.cpu(), act_hn.data.cpu(), agent_masks_record  # NOTE: tensor.data.cpu() is to make the tensor's "is_leaf" = True, this also prevent the error message on line "retain_graph=True"
